<?php 
    session_start(); 
    $name = $nameErr = $username = $userErr = $passErr = $password = $emailErr = $email = $phone = $phoneErr = $address = $addErr =$error = "";
  if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name) && empty($username) && empty($password) && empty($email) && empty($phone) && empty($address)){
      ...
      $checkUser = mysqli_query($conn,$sql); 
             if (mysqli_num_rows($checkUser)>0){
                $error = "<h4>Username đã tồn tại </h4>"; 
                 ...
                 $sql1 = "INSERT INTO `user`(`fullname`, `username`, `password`, `email`, `phone`, `address`) VALUES
               ('$name','$username','$password','$email','$phone','$address')";
                $ketqqua = mysqli_query($conn,$sql1);  
              }                
    }
      }  
}
            ...
            <div class="p-5">
              ...
              <form class="user" action="register.php" method="POST">
                <div class="form-group">
                  <input type="text" class="form-control form-control-user"  placeholder="Fullname" name="name">
                  <?php echo $nameErr;  ?>
                                  </div>
                 <div class="form-group">
